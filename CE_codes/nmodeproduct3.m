function B = nmodeproduct3(A,M,n)
...
% Modified by Anna Kormilitsina to allow unit array sizes, 
% however will only work with 3 dimensional arrays, n = 3
% latest version January 2011.

[n1,n2,n3] = size(A);

[dimvec(1) dimvec(2) dimvec(3)] = size(A);

Asubst = zeros(max(dimvec(1),2),max(dimvec(2),2),max(dimvec(3),2)) ;
% modified here:
Asubst(1:dimvec(1),1:dimvec(2),1:dimvec(3)) = A;
n = fix(n);
...
[dimvecB(1) dimvecB(2) dimvecB(3)]= size(Ash);
dimvecB(1) = size(M,1);
dimvec(n) = size(M,1);
...
% and we re done!
B = B(1:dimvec(1),1:dimvec(2),1:dimvec(3));