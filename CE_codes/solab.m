...
function [f,p,equilibrium] = solab(a,b,nk);
...
if rank(z11)<nk;
    ...
    equilibrium = 0; %f = inf; p = inf;
    f = 0; p = 0;
elseif abs(t(nk,nk))>abs(s(nk,nk)) | abs(t(nk+1,nk+1))<abs(s(nk+1,nk+1));
    %warning('Wrong number of stable eigenvalues.');
    warning('The Equilibrium is Locally Indeterminate')
    equilibrium = 2; %f = inf; p = inf;
    f = 0; p = 0;
else
    z11i = z11\eye(nk);
    s11  = s(1:nk,1:nk);
